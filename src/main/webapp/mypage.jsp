 ...
 <div class="container">
		...
		<div class="adm_list">
			<table border="1" class="adm-table">
				...
				<tr>
					...
					<th scope="col" style="text-align: center;">책제목
					</td>
					<th scope="col">대출일자
					</td>
					<th scope="col">반납예정일
	...
	<section id="tab-section-5" class="tab-section" hidden>
		<div class="adm_list">
			<table border="1" class="adm-table">
				...
				<tr>
					...
					<th scope="col" style="text-align: center;">아이디
					</td>
					<th scope="col" style="text-align: center;">이름
					</td>
					<th scope="col" style="text-align: center;">생일
					</td>
					<th scope="col" style="text-align: center;">주소
					</td>
					<th scope="col" style="text-align: center;">연락처
					</td>
					<th scope="col" style="text-align: center;">회원등급
					</td>
				</tr>
				<%
		...
		for (Users user : userList) {
		    // 로그인한 사용자의 user_level이 10이 아니면서 자신의 정보만 출력
		    if (loginUser.getUser_level()!=10 && user.getId().equals(memberId)) {
		%>
		    <tr>
		        <th scope="col"><input type="checkbox" value="<%=user.getUser_num()%>" data-type="user"></th>
		        <td><%=count%></td>
		        <td><%=user.getId()%></td>
		        <td><%=user.getName()%></td>
		        <td><%=user.getBirth()%></td>
		...
		<%
		    ...
		    // 로그인한 사용자의 user_level이 10인 경우 모든 회원 정보 출력
		    else if (loginUser.getUser_level()==10) {
		%>
		    <tr>
		        <th scope="col"><input type="checkbox" value="<%=user.getUser_num()%>" data-type="user"></th>
		        <td><%=count%></td>
...
<%-- 				%> --%>
	 ...
	 <div class="d-grid gap-3 d-sm-flex justify-content-sm-center justify-content-xl-start">
                      <a class="btn btn-primary btn-lg px-4 me-sm-3" id="return_btn">반납</a>
                      <a class="btn btn-primary btn-lg px-4 me-sm-3" id="update_btn">수정</a>
                      <a class="btn btn-outline-light btn-lg px-4" id="delete_btn">삭제</a>
                      <a class="btn btn-outline-light btn-lg px-4"id="save_btn">저장</a>
                </div>
<!-- 	<div class="btn"> -->
		...
		$(document).on('click', '#return_btn', function(){
			...
			if(count>=2){
			    ...
			    $.ajax({
			        ...
			        data: { mode: "lent_return", isbn : isbn, num: num },
...
// 			            $location.reload();
			            ...
			            console.log('반납 결과가 성공적으로 반영되었습니다.');
			        },
