...
<body>
	...
	<section class="shop container">
		...
		<div class="btn">
			<a href="<%=request.getContextPath()%>/book/insert.jsp"><input
		...
		<div class="shop-content">
			...
			<div class="shop-content">
				<div class="product-box">
					<a
						href="<%=request.getContextPath()%>/book/read.jsp?no=<%=book.getNo()%>"><img
						src="<%=request.getContextPath()%>/img?no=<%=book.getFile_no()%>"
		...
		if (nOfPages > 1) {
		%>
		<%-- 페이지가 1페이지 이상 존재할 때만 유효. --%>
		<ul class="pagination_num">
			<!-- 이전 페이지로 이동하는 버튼. 현재 페이지가 1페이지인 경우는 현재 페이지로 이동하도록 설정. -->
			<li class="page-item <%=currentPage == 1 ? "disabled" : ""%>">
				<a class="page-link"
				href="<%=currentPage == 1 ? "#" : request.getRequestURI() + "?currentPage=" + (currentPage - 1)%>">이전</a>
			</li>
			...
			<li
				class="page-item <%=currentPage == nOfPages ? "disabled" : ""%>">
				<a class="page-link"
				href="<%=currentPage == nOfPages ? "#" : request.getRequestURI() + "?currentPage=" + (currentPage + 1)%>">다음</a>
			</li>
		</ul>
		<%
		}
