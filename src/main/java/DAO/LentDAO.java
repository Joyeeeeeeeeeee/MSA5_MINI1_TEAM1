...
public class LentDAO extends JDBConnection {
	...
	public int lent_insert(Lent lent) {
		...
		String sql = " INSERT INTO lent (LENT_NUM,USER_NUM,ISBN,STATUS,LENT_DATE,LATE_DATE,PLUS_DATE) "
				+ " VALUES(LENT_NUM.NEXTVAL,?, ?, ?, sysdate, sysdate+14, sysdate+14) ";
	...
	// 데이터 삭제
		...
		// 반납
				public int lent_return(int no) {
					int result=0;
					String sql = " UPDATE lent " + " SET status = ?, late_date = sysdate WHERE lent_num = ?";

					try {
						psmt = con.prepareStatement(sql); // 쿼리 실행 객체 생성
						psmt.setString(1, "반납완료"); 
						psmt.setInt(2, no); // 2번 ? 에 게시글 번호를 매핑
